...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1Ijoic2hpdmFtbXVuZGhyYSIsImEiOiJjazRvNXMxcXYxMG9pM29ud256bm9tbjNkIn0.GKN5e33-K264tEogrAcp-w';
  ...
  locations.forEach(function (loc) {
    ...
    el.className = 'marker'; // Place the Marker on the map

    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom'
    }).setLngLat(loc.coordinates).addTo(map); // Add popups for all locations

    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); //Extends the map bounds to include current location
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            url = type === 'password' ? 'http://127.0.0.1:8000/api/v1/users/updatePassword' : 'http://127.0.0.1:8000/api/v1/users/updateCurrentUser';
            _context.next = 4;
            return (0, _axios.default)({
              method: 'PATCH',
              url: url,
...
var stripe = Stripe('pk_test_7i7p3KzLbCV8PaCNUogZrSXy003QFLHZJj');
